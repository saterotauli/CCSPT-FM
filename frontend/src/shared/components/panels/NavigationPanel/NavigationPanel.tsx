...
const NavigationPanel: React.FC<NavigationPanelProps> = ({
  ...
  // Calculate alert counts by level and severity (filtered by active parameter)
  const getAlertCountsByLevel = (level: string) => {
    let levelAlerts = alerts.filter(alert => alert.planta === level);
    
    // Filtrar por parámetro activo si no es 'all'
    if (activeParameter !== 'all') {
      levelAlerts = levelAlerts.filter(alert => alert.paramType === activeParameter);
    }
    
    const baixCount = levelAlerts.filter(alert => alert.severity === 'baix').length;
    ...
    return { baix: baixCount, mitja: mitjaCount, alt: altCount, total: levelAlerts.length };
  };
  return (
    <div style={{
                ...
                {alertCounts.total > 0 && (
                  <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                    ...
                    {alertCounts.baix > 0 && (
                      <span style={{ 
                        fontSize: '11px', 
                        color: ALERT_COLORS.BAIX_TEXT,
                        background: ALERT_COLORS.BAIX,
                        padding: '2px 6px',
                        borderRadius: '10px',
                        fontWeight: 'bold'
                      }}>
                        {alertCounts.baix} baix
                      </span>
                    )}
                  </div>
                )}
