...
// Tipos de severidad
type Severity = 'ok' | 'baix' | 'mitjà' | 'alt';
...
interface SensorHistoryChartProps {
  ...
  // When true, do not render the chart title (useful for embedding in panels)
  hideTitle?: boolean;
  // Controlled time scale (when provided, internal controls can be hidden)
  controlledTimeScale?: 'realtime' | 'hour' | 'day' | 'week' | 'month';
  onTimeScaleChange?: (s: 'realtime' | 'hour' | 'day' | 'week' | 'month') => void;
  hideInlineScaleControls?: boolean;
}
...
const SensorHistoryChart: React.FC<SensorHistoryChartProps> = ({
  ...
  controlledTimeScale,
  onTimeScaleChange,
  hideInlineScaleControls = false,
}) => {
  const [internalTimeScale, setInternalTimeScale] = useState<'realtime' | 'hour' | 'day' | 'week' | 'month'>('realtime');
  const timeScale = controlledTimeScale ?? internalTimeScale;
  const setTimeScale = (s: 'realtime' | 'hour' | 'day' | 'week' | 'month') => {
    if (onTimeScaleChange) onTimeScaleChange(s);
    else setInternalTimeScale(s);
  };
  ...
  // Si overlayInViewer es true, mostrar el panel flotante completo
  if (overlayInViewer) {
    return (
      <div 
  ...
  // Si overlayInViewer es false, renderizar SOLO el contenido para embeber
  return (
    <div style={{ 
      width: '100%', 
      height: '100%', 
      ...
      {/* Controles de escala de tiempo (inline) - ocultables si son externos) */}
      {!(hideInlineScaleControls || controlledTimeScale) && (
        <div style={{
          display: 'flex',
