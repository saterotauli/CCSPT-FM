...
import { setupIfcsClickBlocker } from "../../interactions/Interactions";
import { extractIfcClass, extractName, extractGlobalId } from "../../utils/IfcProps";
import { pickIdWithThree, resolveModelFromHit as resolveModelFromHitExt, resolveElementIdFromHit as resolveElementIdFromHitExt, raycastAll as raycastAllExt, showRayResults as showRayResultsExt } from "../../utils/Raycast";
import { setupCameraDebug } from "../../utils/Debug";
import { setupHover } from "../../interactions/Hover";
import { GeoTracker } from "../../utils/Geolocation";
import { MobileAssetsLayer } from "../../layers/MobileAssetsLayer";

interface BIMViewerProps {
  isMobile: boolean;
  onModelsLoaded?: (components: OBC.Components, world: OBC.World) => Promise<void> | void;
  cleanViewport?: boolean;
}

const BIMViewer: React.FC<BIMViewerProps> = ({ isMobile, onModelsLoaded, cleanViewport }) => {
  ...
  const originSphereRef = useRef<THREE.Object3D | null>(null);
  ...
  useEffect(() => {
    const initializeBIM = async () => {
      ...
      // Measurements cleanup (placeholder, no-op)
      ...
      const resizeWorld = () => {
        ...
        return () => {
        try { if (mobileLayerRef.current) mobileLayerRef.current.stop(); } catch {}
      };
    };
      ...
      // Measurements (length + area): setup and keep cleanup (no-op)
      const measCleanup = setupMeasurements();
      ...
      // Append bottom toolbar + floor selector unless cleanViewport is requested
      if (!cleanViewport) {
      ...
      >(TEMPLATES.contentGridTemplate, {
        ...
        pageMode: 'fm',
        mobile: isMobile,
        onModelsLoaded: async () => {
          if (onModelsLoaded) {
            await onModelsLoaded(components, world);
          } else {
            console.log('BIMViewer: Models loaded (no custom callback provided)');
          }
        },
      });
      ...
      try {
        ...
        console.warn('[MobileAssets] Failed to start layer', e);
      }
    ...
    return () => {
      ...
      try { mobileLayerRef.current?.stop(); } catch {}
      ...
      // Clean up origin sphere
      if (originSphereRef.current) {
        try { 
          const scene = worldRef.current?.scene?.three;
          if (scene) scene.remove(originSphereRef.current);
        } catch {}
        originSphereRef.current = null;
      }
