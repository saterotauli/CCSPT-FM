...
import { useRealTimeSensors } from '../src/hooks/useRealTimeSensors';
import { useModelViewer } from '../src/modules/viewer/hooks/useModelViewer';
import { useIsolation } from '../src/modules/viewer/hooks/useIsolation';
import { useAlerts } from '../src/hooks/useAlerts';
import { useRaycasting } from '../src/modules/viewer/hooks/useRaycasting';
import AlertsPanel from '../src/shared/components/panels/AlertsPanel/AlertsPanel';
import NavigationPanel from '../src/shared/components/panels/NavigationPanel/NavigationPanel';
import AlarmDetailsPanel from '../src/shared/components/panels/AlarmDetailsPanel/AlarmDetailsPanel';
import ViewerRegistry from '../src/modules/viewer/utils/ViewerRegistry';
...
const ModelViewerRefactored: React.FC = () => {
  ...
  const [severityFilter, setSeverityFilter] = useState<Array<'ok'|'baix'|'mitjà'|'alt'>>(['alt', 'mitjà', 'baix', 'ok']);
  ...
  const generateHistoricalData = useCallback((sensor: any) => {
    const now = new Date();
    const data: Array<{ timestamp: Date; temperature: number; humidity: number; ppm: number }> = [];
  ...
  // Generate alerts when sensor data, parameter or severity filter changes
  useEffect(() => {
    generateAlerts();
  }, [sensorData, code, activeParameter, buildingColor, severityFilter, generateAlerts]);
  ...
  useEffect(() => {
    const initializeViewer = async () => {
        ...
        try {
          ...
          if (!highlighterRef.current) {
            ...
            highlighter.styles.set("baix", {
              color: new THREE.Color("#FFFF2E"), // Amarillo dorado
              opacity: 1.0,
              transparent: false,
              renderedFaces: 0,
            });
            ...
            highlighter.styles.set("mitja", {
              color: new THREE.Color("#FFAE45"), // Naranja
              opacity: 1.0,
              transparent: false,
              renderedFaces: 0,
            });
            ...
            highlighter.styles.set("alt", {
              color: new THREE.Color("#ff0000"), // Rojo
              opacity: 1.0,
              transparent: false,
              renderedFaces: 0,
            });
            ...
            highlighter.styles.set("ok", {
              color: new THREE.Color("#00FF00"), // Verde
              opacity: 1.0,
              transparent: false,
              renderedFaces: 0,
            });
          ...
          setTimeout(() => {
            try {
              ...
              if (!highlighterRef.current) {
                ...
                // Create custom highlight styles for alerts (retry)
                highlighter.styles.set("baix", {
                  color: new THREE.Color("#FFFF2E"), // Amarillo dorado
                  opacity: 1.0,
                  transparent: false,
      ...
      const modelPath = `/models/CCSPT-${code || 'ALB'}-M3D-Rooms.frag`;
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          {/* Floating top-right: parameter toggles */}
          <div style={{ position: 'absolute', top: 12, right: 12, zIndex: 5, display: 'flex', gap: 10 }}>
            <button
              onClick={() => setActiveParameter('temperatura')}
              title="Temperatura"
              style={{
                width: 40, height: 40, borderRadius: '50%',
                border: '1px solid #e0e6ef', cursor: 'pointer',
                background: activeParameter === 'temperatura' ? '#4179b5' : '#ffffff',
                color: activeParameter === 'temperatura' ? '#ffffff' : '#111827'
              }}
            >
              🌡️
            </button>
            <button
              ...
              style={{
                width: 40, height: 40, borderRadius: '50%',
                border: '1px solid #e0e6ef', cursor: 'pointer',
                background: activeParameter === 'humitat' ? '#4179b5' : '#ffffff',
                color: activeParameter === 'humitat' ? '#ffffff' : '#111827'
              }}
            >
              💧
            </button>
            <button
              ...
              style={{
                width: 40, height: 40, borderRadius: '50%',
                border: '1px solid #e0e6ef', cursor: 'pointer',
                background: activeParameter === 'ppm' ? '#4179b5' : '#ffffff',
                color: activeParameter === 'ppm' ? '#ffffff' : '#111827'
              }}
            >
              ⚗️
            </button>
