...
exports.sensorSimulationService = void 0;
...
class SensorSimulationService {
    constructor() {
        ...
        this.UPDATE_INTERVAL = parseInt(process.env.SENSOR_UPDATE_INTERVAL || '120000'); // 2 minutos por defecto (optimizado)
        this.MAX_ROOMS_PER_BATCH = parseInt(process.env.SENSOR_BATCH_SIZE || '25'); // Procesar máximo 25 habitaciones por vez (optimizado)
        this.ENABLE_BATCH_MODE = process.env.SENSOR_BATCH_MODE !== 'false'; // Modo lote habilitado por defecto
     ...
     */
    getBaseValuesForRoomType(roomType) {
    ...
    generateSensorReading(state) {
        ...
        const temperature = Math.max(15, Math.min(30, state.baseTemperature +
            timeVariation +
            randomVariation +
            state.temperatureTrend * timeSinceLastUpdate));
        const humidity = Math.max(20, Math.min(80, state.baseHumidity +
            (timeVariation * 2) +
            (randomVariation * 1.5) +
            state.humidityTrend * timeSinceLastUpdate));
        const ppm = Math.max(300, Math.min(1000, state.basePpm +
            (randomVariation * 20) +
     ...
     */
    async saveSensorReading(spaceGuid, reading) {
        if (!this.dbAvailable)
            return; // Si no hay BD, no persistimos pero seguimos simulando
        try {
     ...
     */
    async processSimulationIteration() {
    ...
    async processFullSimulation() {
        ...
        const savePromises = readings.map(({ spaceGuid, reading }) => this.saveSensorReading(spaceGuid, reading));
        try {
     ...
     */
    selectRandomRooms(roomEntries, count) {
     ...
     */
    async getLatestSensorData(spaceGuid, limit = 10) {
        try {
...
// Instancia singleton
exports.sensorSimulationService = new SensorSimulationService();
