...
exports.ephemeralSensorService = void 0;
...
class EphemeralSensorService {
    ...
    generateSensorReading(state) {
        ...
        const temperature = Math.max(15, Math.min(35, state.baseTemperature +
            timeVariation +
            randomVariation +
            state.temperatureTrend * timeSinceLastUpdate));
        const humidity = Math.max(25, Math.min(85, state.baseHumidity +
            (timeVariation * 3) +
            (randomVariation * 2) +
            state.humidityTrend * timeSinceLastUpdate));
        const ppm = Math.max(200, Math.min(1200, state.basePpm +
            (randomVariation * 40) +
     ...
     */
    async getCurrentSensorReading(spaceGuid) {
     ...
     */
    async getCurrentSensorReadings(spaceGuids) {
        await this.initializeRoomStates();
     ...
     */
    async getAllCurrentSensorReadings() {
        await this.initializeRoomStates();
        const readings = [];
     ...
     */
    async getSensorHistory(spaceGuid, count = 10) {
        await this.initializeRoomStates();
        const state = this.roomStates.get(spaceGuid);
        if (!state)
            return [];
...
// Instancia singleton
exports.ephemeralSensorService = new EphemeralSensorService();
