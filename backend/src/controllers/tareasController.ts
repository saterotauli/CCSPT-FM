...
export const getTareas = async (req: AuthRequest, res: Response) => {
  try {
    const { 
      estado, 
      prioridad, 
    ...
    if (asignadoA) where.asignadoAId = Number(asignadoA);
    if (creadoPor) where.creadoPorId = Number(creadoPor);
    if (edifici) where.edifici = edifici;
...
export const getTarea = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
export const createTarea = async (req: AuthRequest, res: Response) => {
  try {
    const {
      titulo,
      descripcion,
    ...
    if (asignadoAId) {
      await Promise.all([
        ...
        FirebaseService.sendNotificationToUser(Number(asignadoAId), {
...
export const updateTarea = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const {
      titulo,
...
export const deleteTarea = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
export const getMisTareas = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'No autenticado' });
    }
    ...
    const where: any = { asignadoAId: req.user.id };
    if (estado) where.estado = estado;
